...
static void bench_seq_rw(void *map, size_t filesize, size_t bs) {
    if (bs == 0 || bs > filesize) {
        fprintf(stderr, "Skipping block size %zu: invalid for filesize %zu\n", bs, filesize);
        return;
    }
    ...
    if (ops == 0) {
        fprintf(stderr, "Skipping block size %zu: 0 ops for filesize %zu\n", bs, filesize);
        free(buf);
        return;
    }
    ...
    printf("MMAP_WRITE_SEQ, bs=%zu, total_bytes=%zu, elapsed=%.6f sec, throughput=%.2f MiB/s, iops=%.2f\n",
    ...
    printf("MMAP_READ_SEQ,  bs=%zu, total_bytes=%zu, elapsed=%.6f sec, throughput=%.2f MiB/s, iops=%.2f\n",
...
static void bench_rand_write(void *map, size_t filesize, size_t bs, double duration_sec) {
    if (bs == 0 || bs > filesize) {
        fprintf(stderr, "Skipping rand write bs=%zu: invalid for filesize %zu\n", bs, filesize);
        return;
    }
    size_t align = bs;
    ...
    if (filesize < bs || max_index == (size_t)-1) {
        fprintf(stderr, "Skipping rand write bs=%zu: filesize too small\n", bs);
        return;
    }
    ...
    for (;;) {
        ...
        timespec_now(&now);
        if (elapsed_sec(start, now) >= duration_sec) break;
    }
    ...
    printf("MMAP_RAND_WRITE_%us, bs=%zu, ran_seconds=%.3f, ops=%llu, bytes=%llu, throughput=%.2f MiB/s, iops=%.2f\n",
           (unsigned)duration_sec, bs, secs, (unsigned long long)ops, bytes, mbps, iops);
...
static void bench_rand_read(void *map, size_t filesize, size_t bs, double duration_sec) {
    if (bs == 0 || bs > filesize) {
        fprintf(stderr, "Skipping rand read bs=%zu: invalid for filesize %zu\n", bs, filesize);
        return;
    }
    size_t align = bs;
    if (align < 4096) align = 4096;
    ...
    size_t max_index = (filesize - bs) / align;
    if (filesize < bs || max_index == (size_t)-1) {
        fprintf(stderr, "Skipping rand read bs=%zu: filesize too small\n", bs);
        return;
    }
    ...
    double mbps = (bytes/1024.0/1024.0) / secs;
    double iops = ops / secs;
    printf("MMAP_RAND_READ_%us,  bs=%zu, ran_seconds=%.3f, ops=%llu, bytes=%llu, throughput=%.2f MiB/s, iops=%.2f\n",
           (unsigned)duration_sec, bs, secs, (unsigned long long)ops, bytes, mbps, iops);
...
int main(int argc, char **argv) {
    if (argc < 2) {
        ...
        fprintf(stderr, "Example: %s /mnt/famfs/mmap_100GB.bin 4K,64K,1M\n", argv[0]);
        return 1;
    }
    ...
    if (filesize == 0) {
        fprintf(stderr, "File size is 0; please create and size the file before running: %s\n", path);
        close(fd);
        return 5;
    }
    ...
    if (rv != 0) {
        fprintf(stderr, "posix_madvise(POSIX_MADV_SEQUENTIAL) returned %d\n", rv);
    }
    ...
    printf("MMAP_BENCH_BEGIN, file=%s, size_bytes=%zu, sizes_csv=%s, rand_secs=%.3f\n",
