...
# Default device is /dev/dax0.0 (user can override by setting FAMFS_DEV in cmdline)
...
MMAP_SIZES="${MMAP_SIZES:-4K,64K,1M}"
...
# Do not reformat and mount, re-use the files created in prev test
  ensure_mounted
...
test_006_mmap_100GB() {
  reformat_and_mount
  ensure_mounted
  measure "CREATE_100GB" famfs_create "$MOUNT_DIR/mmap_${MMAP_SIZE_GB}GB.bin" $((100 * 1024 * 1024 * 1024))
  echo "[TEST] ${MMAP_SIZE_GB}GB mmap read/write bandwidth & IOPS (${MMAP_SIZES})"
  measure "MMAP_${MMAP_SIZE_GB}GB" "${WORK_DIR}/mmap_bench" "$MOUNT_DIR/mmap_${MMAP_SIZE_GB}GB.bin" "$MMAP_SIZES"
}
...
register_test "006" "${MMAP_SIZE_GB}GB mmap BW & IOPS" test_006_mmap_100GB
