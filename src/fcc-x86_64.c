...
static fcc_func_ptr flush_cacheline_func;
static fcc_func_ptr invalidate_cacheline_func;
static fence_fn_t fence_sfunc;
static fence_fn_t fence_mfunc;
static fence_fn_t fence_lfunc;
...
static void x86_init_flush_functions(void)
{
	...
	fence_sfunc = x86_sfence;
	fence_mfunc = x86_mfence;
	fence_lfunc = x86_lfence;
}
...
void invalidate_processor_cache(const void *addr, size_t len)
{
	...
	famfs_log(FAMFS_LOG_DEBUG, "%s 0x%" PRIxPTR " %lu\n", __func__,
			(uintptr_t)addr, len);
