...
TEST(famfs, famfs_log)
{
	...
	rc = famfs_fsck("/tmp/famfs/.meta/.superblock", 1 /* mmap */, 1, 0, 0, 1);
	ASSERT_EQ(rc, 0);
	rc = famfs_fsck("/tmp/famfs/.meta/.superblock", 0 /* read */, 1, 0, 0, 1);
	ASSERT_EQ(rc, 0);
	rc = famfs_fsck("/tmp/nonexistent-file", 0 /* read */, 1, 0, 0, 1);
	ASSERT_NE(rc, 0);
	...
	rc = famfs_fsck("/tmp/famfs/.meta/.log", 1 /* mmap */, 1, 0, 0, 1);
	ASSERT_NE(rc, 0);
	rc = famfs_fsck("/tmp/famfs/.meta/.log", 0 /* read */, 1, 0, 0, 1);
	ASSERT_NE(rc, 0);
...
#define NBUCKETS 10000
...
#if 0
	printf("After finding root inode:\n");
	dump_icache(&icache);
#endif
	...
	while ((inode_num = bucket_series_next(bs)) != -1) {
		...
		loopct++;
#if 0
		printf("inum=%lld num_deleted=%lld loopct=%lld\n",
		       inode_num, num_in_icache, loopct);
#endif
		/* Put one ref for the find above, and one to "free" the inode */
		famfs_inode_putref_locked(inode);
		famfs_inode_putref_locked(inode);
#if 0
		printf("After putref on ino=%lld:\n", inode_num);
		dump_icache(&icache);
#endif
